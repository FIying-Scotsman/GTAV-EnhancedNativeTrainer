...
void begin_prop_placement(SpawnedPropInstance prop)
{
	...
	if (ENTITY::IS_ENTITY_DEAD(playerPed, FALSE) || !bPlayerExists || !propExists)
	{
		return;
	}
	...
	float tempFloat = CAMERA::GET_GAMEPLAY_CAM_RELATIVE_PITCH();
	CAMERA::SET_GAMEPLAY_CAM_RELATIVE_HEADING(0.0f);
	CAMERA::SET_GAMEPLAY_CAM_RELATIVE_PITCH(tempFloat, 0.0f);
	...
	Vector3 minDimens;
	Vector3 maxDimens;
	MISC::GET_MODEL_DIMENSIONS(modelHash, &minDimens, &maxDimens);
	...
	propCamera = CAMERA::CREATE_CAM("DEFAULT_SCRIPTED_CAMERA", 0);
	Vector3 worldCamCoords = ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(currentProp.instance, 0.0f, -cameraDistance, 2.0f);
	CAMERA::SET_CAM_COORD(propCamera, worldCamCoords.x, worldCamCoords.y, worldCamCoords.z);
	CAMERA::POINT_CAM_AT_ENTITY(propCamera, currentProp.instance, 0.0f, 0.0f, 0.0f, 1);
	
	CAMERA::SET_GAMEPLAY_CAM_RELATIVE_PITCH(20.0f, 0x3f800000);
	CAMERA::SET_CAM_ACTIVE(propCamera, 1);
	CAMERA::RENDER_SCRIPT_CAMS(1, 0, 3000, 1, 0, 0);

	if (CAMERA::GET_FOLLOW_PED_CAM_VIEW_MODE() == 4) {
		first_cam = true;
		CAMERA::SET_FOLLOW_PED_CAM_VIEW_MODE(0);
	}

	//check it worked
	if (!CAMERA::DOES_CAM_EXIST(propCamera))
	{
	...
	while (true && !pp_exit_flag)
	{
		...
		BOOL camExists = CAMERA::DOES_CAM_EXIST(propCamera);

		//Disable on object death
		if (ENTITY::IS_ENTITY_DEAD(playerPed, FALSE) || !bPlayerExists || !propExists || !camExists)
		{
			pp_exit_flag = true;
			break;
		}
		...
		ss << "Pitch: " << CAMERA::GET_GAMEPLAY_CAM_RELATIVE_PITCH() << ", Heading:" << CAMERA::GET_GAMEPLAY_CAM_RELATIVE_HEADING();
		...
		*/
		...
		float camPitch = CAMERA::GET_GAMEPLAY_CAM_RELATIVE_PITCH();
		float camHeading = CAMERA::GET_GAMEPLAY_CAM_RELATIVE_HEADING();

		CAMERA::SET_THIRD_PERSON_CAM_RELATIVE_PITCH_LIMITS_THIS_UPDATE(-90.0f, 90.0f);
		...
		bool groundFound = MISC::GET_GROUND_Z_FOR_3D_COORD(newCamCoords.x, newCamCoords.y, entityCoords.z + 3.0f, &groundZ, FALSE, FALSE) == 1;
		if (groundFound && groundZ > newCamCoords.z - aboveGroundMargin)
		{
			...
			CAMERA::SET_THIRD_PERSON_CAM_RELATIVE_PITCH_LIMITS_THIS_UPDATE(limitedCamAngle, 90.0f);
		}

		CAMERA::SET_CAM_COORD(propCamera, newCamCoords.x, newCamCoords.y, newCamCoords.z);
		CAMERA::POINT_CAM_AT_ENTITY(propCamera, currentProp.instance, 0.0f, 0.0f, 0.0f, 1);
	...
	CAMERA::RENDER_SCRIPT_CAMS(0, 0, 3000, 1, 0, 0);
	if (CAMERA::DOES_CAM_EXIST(propCamera))
	{
		CAMERA::DESTROY_CAM(propCamera, 1);
	}
	...
	if (GetTickCount() < propPlacerStatusTextDrawTicksMax)
	{
		...
		for (int i = 0; i < numLines; i++)
		{
			...
			if (i == 0 || i == 10 || i >= 17)
			{
				...
				HUD::SET_TEXT_COLOUR(255, 180, 0, 255);
			}
			else
			{
			...
			HUD::SET_TEXT_DROPSHADOW(1, 1, 1, 1, 1);
			HUD::SET_TEXT_EDGE(1, 0, 0, 0, 305);
			if (propPlacerStatusTextGxtEntry)
			{
				HUD::BEGIN_TEXT_COMMAND_DISPLAY_TEXT((char *)propPlacerStatusLines.at(i).c_str());
			}
			else
			{
				HUD::BEGIN_TEXT_COMMAND_DISPLAY_TEXT("STRING");
				HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME((char *)propPlacerStatusLines.at(i).c_str());
			}
			HUD::END_TEXT_COMMAND_DISPLAY_TEXT(0.01, textY, 0);
...
void prop_placement()
{
	...
	if (IsKeyJustUp(KeyConfig::KEY_OBJECTPLACER_HELP) || PAD::IS_DISABLED_CONTROL_JUST_PRESSED(2, INPUT_FRONTEND_SELECT))
	{
	...
	if (secondaryMove)
	{
		...
		float mouseX = PAD::GET_CONTROL_NORMAL(2, 239);
		float mouseY = PAD::GET_CONTROL_NORMAL(2, 240);
