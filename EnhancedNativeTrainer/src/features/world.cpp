...
bool onconfirm_weather_menu(MenuItem<std::string> choice)
{
	...
	default:
		...
		if (!lastWeather.empty()) {
			...
			MISC::SET_WEATHER_TYPE_NOW((char *)lastWeather.c_str());
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3061285535) MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
			if (WORLD_LIGHTNING_INTENSITY_VALUES[featureLightIntensityIndex] > -2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3373937154) MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
			if (screenfltr == "DEFAULT" || screenfltr == "") GRAPHICS::CLEAR_TIMECYCLE_MODIFIER();
		}
...
void process_weather_menu()
{
	...
	StringStandardOrToggleMenuDef lines[] = {
	...
	// Was a hardcoded 19 with only 18 entries listed above - the phantom 19th slot got
	// zero-initialised (empty caption, isLeaf false) instead of an intended menu entry,
	// which is exactly the blank "Weather: " item you saw at the bottom of the list:
	// selecting it hit onconfirm_weather_menu's default case with an empty
	// choice.value/caption, skipped the (empty) weather-setting call, and fell through
	// to the unconditional set_status_text() with nothing appended.
...
bool onconfirm_clouds_menu(MenuItem<std::string> choice)
{
	...
	case 1:
		// No Clouds
		lastClouds = "Snowy 01";
		MISC::LOAD_CLOUD_HAT((char*)lastClouds.c_str(), 0.3);
		WAIT(10);
		lastClouds = "";
		lastCloudsName = "";
		MISC::UNLOAD_ALL_CLOUD_HATS();
...
void update_world_features()
{
	...
	{
		GRAPHICS::SET_ARTIFICIAL_LIGHTS_STATE(featureBlackout.enabled);
	...
	if (featureWorldNoPeds)
	{
		...
		MISC::CLEAR_AREA_OF_PEDS(v3.x, v3.y, v3.z, 1000.0, 0);
	...
	// Show Full Map
	if (featureFullMap) HUD::SET_MINIMAP_HIDE_FOW(true); 
	if (!featureFullMap) HUD::SET_MINIMAP_HIDE_FOW(false);
	...
	if (featurePenitentiaryMap.enabled || featureCayoPericoMap.enabled) {
		...
		if (INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_me.x, coords_me.y, coords_me.z)) {
			if (featurePenitentiaryMap.enabled) HUD::SET_RADAR_AS_INTERIOR_THIS_FRAME(MISC::GET_HASH_KEY("V_FakePrison"), 1700, 2580, 0, 0);
			if (featureCayoPericoMap.enabled) HUD::SET_RADAR_AS_INTERIOR_THIS_FRAME(MISC::GET_HASH_KEY("h4_fake_islandx"), 4700.0f, -5145.0, 0, 0); // THANKS TO SJAAK327 FOR THE CODE
	...
	if (MISC_WEATHER_CHANGE_VALUES[WeatherChangeIndex] > 0 && !featureWeatherFreeze.enabled) {
		...
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 2544503417) cur_w = "EXTRASUNNY";
		if (random_weather == 2) rand_w1 = "CLEAR";
		if (random_weather2 == 2) rand_w2 = "CLEAR";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 916995460) cur_w = "CLEAR";
		if (random_weather == 3) rand_w1 = "CLOUDS";
		if (random_weather2 == 3) rand_w2 = "CLOUDS";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 821931868) cur_w = "CLOUDS";
		if (random_weather == 4) rand_w1 = "SMOG";
		if (random_weather2 == 4) rand_w2 = "SMOG";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 282916021) cur_w = "SMOG";
		if (random_weather == 5) rand_w1 = "FOGGY";
		if (random_weather2 == 5) rand_w2 = "FOGGY";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 2926802500) cur_w = "FOGGY";
		if (random_weather == 6) rand_w1 = "OVERCAST";
		if (random_weather2 == 6) rand_w2 = "OVERCAST";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3146353965) cur_w = "OVERCAST";
		if (random_weather == 7) rand_w1 = "RAIN";
		if (random_weather2 == 7) rand_w2 = "RAIN";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 1420204096) cur_w = "RAIN";
		if (random_weather == 8) rand_w1 = "THUNDER";
		if (random_weather2 == 8) rand_w2 = "THUNDER";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3061285535) cur_w = "THUNDER";
		if (random_weather == 9) rand_w1 = "CLEARING";
		if (random_weather2 == 9) rand_w2 = "CLEARING";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 1840358669) cur_w = "CLEARING";
		if (random_weather == 10) rand_w1 = "NEUTRAL";
		if (random_weather2 == 10) rand_w2 = "NEUTRAL";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 2764706598) cur_w = "NEUTRAL";
		if (random_weather == 11) rand_w1 = "SNOW";
		if (random_weather2 == 11) rand_w2 = "SNOW";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 4021743606) cur_w = "SNOW";
		if (random_weather == 12) rand_w1 = "BLIZZARD";
		if (random_weather2 == 12) rand_w2 = "BLIZZARD";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 669657108) cur_w = "BLIZZARD";
		if (random_weather == 13) rand_w1 = "SNOWLIGHT";
		if (random_weather2 == 13) rand_w2 = "SNOWLIGHT";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 603685163) cur_w = "SNOWLIGHT";
		if (random_weather == 14) rand_w1 = "XMAS";
		if (random_weather2 == 14) rand_w2 = "XMAS";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 2865350805) cur_w = "XMAS";
		if (random_weather == 15) rand_w1 = "HALLOWEEN";
		if (random_weather2 == 15) rand_w2 = "HALLOWEEN";
		if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] != 2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3373937154) cur_w = "HALLOWEEN";
		...
		if (startup_w == false) {
			...
			if (screenfltr == "DEFAULT" || screenfltr == "") GRAPHICS::CLEAR_TIMECYCLE_MODIFIER();
			MISC::UNLOAD_ALL_CLOUD_HATS();
			if (!lastWeather.empty() && (lastWeather == "THUNDER" || MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("THUNDER"))) MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
			if (WORLD_LIGHTNING_INTENSITY_VALUES[featureLightIntensityIndex] > -2 && !lastWeather.empty() && (lastWeather == "HALLOWEEN" || MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("THUNDER")))
				MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
			lastWeather = cur_w;
			lastWeather_2 = cur_w;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("EXTRASUNNY")) alphabetical_w = 1;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("CLEAR")) alphabetical_w = 2;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("CLOUDS")) alphabetical_w = 3;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("SMOG")) alphabetical_w = 4;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("FOGGY")) alphabetical_w = 5;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("OVERCAST")) alphabetical_w = 6;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("RAIN")) alphabetical_w = 7;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("THUNDER")) alphabetical_w = 8;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("CLEARING")) alphabetical_w = 9;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("NEUTRAL")) alphabetical_w = 10;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("SNOW")) alphabetical_w = 11;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("BLIZZARD")) alphabetical_w = 12;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("SNOWLIGHT")) alphabetical_w = 13;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("XMAS")) alphabetical_w = 14;
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == MISC::GET_HASH_KEY("HALLOWEEN")) alphabetical_w = 15;
		...
		if (startup_w == true) {
			...
			if (w_seconds > (MISC_WEATHER_CHANGE_VALUES[WeatherChangeIndex] - 5)) {
				if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] == 0) {
					...
					if (t_counter > 0.000) {
						MISC::SET_CURR_WEATHER_STATE(MISC::GET_HASH_KEY((char*)mixed_w1.c_str()), MISC::GET_HASH_KEY((char*)lastWeather.c_str()), t_counter);
				...
				if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] == 1) {
					...
					if (t_counter > 0.0) {
						MISC::SET_CURR_WEATHER_STATE(MISC::GET_HASH_KEY((char*)cur_w.c_str()), MISC::GET_HASH_KEY((char*)mixed_w1.c_str()), t_counter);
						MISC::SET_CURR_WEATHER_STATE(MISC::GET_HASH_KEY((char*)cur_w.c_str()), MISC::GET_HASH_KEY((char*)mixed_w2.c_str()), t_counter);
				...
				if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] == 2) {
					...
					if (t_counter > 0.000) {
						MISC::SET_CURR_WEATHER_STATE(MISC::GET_HASH_KEY((char*)lastWeather.c_str()), MISC::GET_HASH_KEY((char*)cur_w.c_str()), t_counter);
						t_counter = t_counter + 0.001;
					}
				}
				...
				if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] == 0 && t_counter > 0.065 && t_counter < 0.075) {
					if (lastWeather == "EXTRASUNNY" || lastWeather == "NEUTRAL") MISC::LOAD_CLOUD_HAT("Snowy 01", 0.3); // 9.0
					if (lastWeather == "CLOUDS" && weather_counter == 1) MISC::LOAD_CLOUD_HAT("shower", 0.3); // 9.0
					if (lastWeather == "CLOUDS" && weather_counter == 2) MISC::LOAD_CLOUD_HAT("stratoscumulus", 0.3); // 9.0
					if (lastWeather == "THUNDER") MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3); // 9.0
					if (WORLD_LIGHTNING_INTENSITY_VALUES[featureLightIntensityIndex] > -2 && lastWeather == "HALLOWEEN") MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3); // 9.0
				}
				...
				if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] == 1 && t_counter > 0.55 && t_counter < 0.65) {
					if (mixed_w1 == "EXTRASUNNY" || mixed_w2 == "EXTRASUNNY" || mixed_w1 == "NEUTRAL" || mixed_w2 == "NEUTRAL") MISC::LOAD_CLOUD_HAT("Snowy 01", 0.3);
					if (mixed_w1 == "THUNDER" || mixed_w2 == "THUNDER") MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
					if (WORLD_LIGHTNING_INTENSITY_VALUES[featureLightIntensityIndex] > -2 && (mixed_w1 == "HALLOWEEN" || mixed_w2 == "HALLOWEEN")) MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
				}
				if (NPC_RAGDOLL_VALUES[WeatherMethodIndexN] == 2 && t_counter > 0.065 && t_counter < 0.075) {
					...
					if (cur_w == "EXTRASUNNY" || cur_w == "NEUTRAL") MISC::LOAD_CLOUD_HAT("Snowy 01", 0.3); // 9.0
					if (cur_w == "THUNDER") MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3); // 9.0
					if (WORLD_LIGHTNING_INTENSITY_VALUES[featureLightIntensityIndex] > -2 && cur_w == "HALLOWEEN") MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3); // 9.0
				}
	...
	if (featureSnow.enabled) {
		...
		if (winter_water_tick < 7000) MISC::WATER_OVERRIDE_SET_STRENGTH(3.0f); // 10000
		if (winter_water_tick > 6999 && winter_water_tick < 7300) MISC::WATER_OVERRIDE_SET_STRENGTH(0.0f); // 9999 10300
	...
	if (WORLD_WAVES_VALUES[WorldWavesIndex] != -1 && WORLD_WAVES_VALUES[WorldWavesIndex] != -2) WATER::SET_DEEP_OCEAN_SCALER(WORLD_WAVES_VALUES[WorldWavesIndex]);
	if (WORLD_WAVES_VALUES[WorldWavesIndex] != -1 && WORLD_WAVES_VALUES[WorldWavesIndex] == -2) MISC::WATER_OVERRIDE_SET_STRENGTH(1.0f);
	
	// Lightning Intensity
	if (WORLD_LIGHTNING_INTENSITY_VALUES[featureLightIntensityIndex] > -2 && (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3061285535 || MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3373937154)) { // GET_NEXT_WEATHER_TYPE_HASH_NAME
	...
	if (featureBusLight || featureNPCNoLights || featureNPCNeonLights || featureDirtyVehicles || featureCleanVehicles ||featureNPCNoGravityVehicles || featureNPCReducedGripVehicles ||
		...
		for (int i = 0; i < found; i++) {
			...
			if (featureBusLight && VEHICLE::GET_IS_VEHICLE_ENGINE_RUNNING(bus_veh[i])) {
				...
				if (currVehModel == MISC::GET_HASH_KEY("BUS") || currVehModel == MISC::GET_HASH_KEY("RENTALBUS") || currVehModel == MISC::GET_HASH_KEY("TOURBUS") || currVehModel == MISC::GET_HASH_KEY("COACH") ||
					currVehModel == MISC::GET_HASH_KEY("AIRBUS")) {
					BOOL lightsOn = -1;
					BOOL highbeamsOn = -1;
					...
					if (lightsOn || highbeamsOn) {
						...
						if (currVehModel != MISC::GET_HASH_KEY("COACH")) GRAPHICS::DRAW_SPOT_LIGHT(bone1_coord.x, bone1_coord.y, bone1_coord.z, dirVector_lf_lr_x, dirVector_lf_lr_y, dirVector_lf_lr_z, 255, 255, 255, 1.0, 400, 50, 930, 900);
						...
						if (currVehModel != MISC::GET_HASH_KEY("COACH")) GRAPHICS::DRAW_SPOT_LIGHT(bone7_coord.x, bone7_coord.y, bone7_coord.z + 1, dirVector_lf_lr_x, dirVector_lf_lr_y, dirVector_lf_lr_z, 255, 255, 255, 2.0, 400, 50, 930, 900);
			...
			// Train Speed
			if (WORLD_TRAIN_SPEED_VALUES[TrainSpeedIndex] != -1.0 && VEHICLE::GET_VEHICLE_CLASS(bus_veh[i]) == 21 && VEHICLE::GET_IS_VEHICLE_ENGINE_RUNNING(bus_veh[i]) && MISC::GET_MISSION_FLAG() == 0) { // Train Speed
			...
			if (featureNPCNeonLights) {
				if (ENTITY::DOES_ENTITY_EXIST(bus_veh[i]) && bus_veh[i] != veh_mycurrveh && !VEHICLE::GET_VEHICLE_NEON_ENABLED(bus_veh[i], 0) && !VEHICLE::GET_VEHICLE_NEON_ENABLED(bus_veh[i], 1) && !VEHICLE::GET_VEHICLE_NEON_ENABLED(bus_veh[i], 2) &&
					!VEHICLE::GET_VEHICLE_NEON_ENABLED(bus_veh[i], 3)) {
					...
					if (VEHICLE::IS_THIS_MODEL_A_CAR(currVehModel_neon)) {
						...
						VEHICLE::SET_VEHICLE_NEON_COLOUR(bus_veh[i], npc_whichcolor.rVal, npc_whichcolor.gVal, npc_whichcolor.bVal);
						VEHICLE::SET_VEHICLE_NEON_ENABLED(bus_veh[i], 0, true);
						VEHICLE::SET_VEHICLE_NEON_ENABLED(bus_veh[i], 1, true);
						//
						VEHICLE::SET_VEHICLE_NEON_ENABLED(bus_veh[i], 2, true);
						VEHICLE::SET_VEHICLE_NEON_ENABLED(bus_veh[i], 3, true); 
						//
						if (getGameVersion() > 45) {
							VEHICLE::SET_VEHICLE_XENON_LIGHT_COLOR_INDEX(bus_veh[i], temp_colour);
			...
			if (featureNPCNoGravityVehicles) {
				...
				if (bus_veh[i] != veh_mycurrveh && bus_veh[i] != veh_i_last_used) {
					...
					float height_n_g = -1;
					MISC::GET_GROUND_Z_FOR_3D_COORD(veh_no_gr.x, veh_no_gr.y, veh_no_gr.z, &height_n_g, FALSE, FALSE);
			...
			if (VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripSnowingCustomIndex] > 0 && featureSnow.enabled) {
				...
				if (slippery_s < slip_index_s && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_slip_ped.x, coords_slip_ped.y, coords_slip_ped.z)) VEHICLE::SET_VEHICLE_REDUCE_GRIP(bus_veh[i], true);
				...
				// arcade
				if (VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripSnowingCustomIndex] < 6 && slippery_randomize > 990 && !TASK::IS_PED_STILL(PLAYER::PLAYER_PED_ID()) && !PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), false) &&
					TASK::IS_PED_RUNNING(PLAYER::PLAYER_PED_ID()) && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_slip.x, coords_slip.y, coords_slip.z) && !ENTITY::IS_ENTITY_IN_WATER(PLAYER::PLAYER_PED_ID()))
					PED::SET_PED_TO_RAGDOLL(PLAYER::PLAYER_PED_ID(), time11, time12, r_Type, true, true, false);
				if (VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripSnowingCustomIndex] < 6 && slippery_randomize > 960 && !TASK::IS_PED_STILL(PLAYER::PLAYER_PED_ID()) && !PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), false) &&
					TASK::IS_PED_SPRINTING(PLAYER::PLAYER_PED_ID()) && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_slip.x, coords_slip.y, coords_slip.z) && !ENTITY::IS_ENTITY_IN_WATER(PLAYER::PLAYER_PED_ID())) 
					PED::SET_PED_TO_RAGDOLL(PLAYER::PLAYER_PED_ID(), time11, time12, r_Type, true, true, false);
				// realistic
				if (VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripSnowingCustomIndex] > 5 && slippery_randomize > 980 && !TASK::IS_PED_STILL(PLAYER::PLAYER_PED_ID()) && !PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), false) &&
					TASK::IS_PED_RUNNING(PLAYER::PLAYER_PED_ID()) && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_slip.x, coords_slip.y, coords_slip.z) && !ENTITY::IS_ENTITY_IN_WATER(PLAYER::PLAYER_PED_ID())) 
					PED::SET_PED_TO_RAGDOLL(PLAYER::PLAYER_PED_ID(), time11, time12, r_Type, true, true, false);
				if (VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripSnowingCustomIndex] > 5 && slippery_randomize > 930 && !TASK::IS_PED_STILL(PLAYER::PLAYER_PED_ID()) && !PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), false) &&
					TASK::IS_PED_SPRINTING(PLAYER::PLAYER_PED_ID()) && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_slip.x, coords_slip.y, coords_slip.z) && !ENTITY::IS_ENTITY_IN_WATER(PLAYER::PLAYER_PED_ID())) 
					PED::SET_PED_TO_RAGDOLL(PLAYER::PLAYER_PED_ID(), time11, time12, r_Type, true, true, false);
				// normalize speed
				Vector3 my_coords_s = ENTITY::GET_ENTITY_COORDS(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0), true);
				if (VEH_ENG_POW_VALUES[engPowMultIndex] < 0 && VEHICLE::IS_THIS_MODEL_A_CAR(ENTITY::GET_ENTITY_MODEL(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0))) && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(my_coords_s.x, my_coords_s.y, my_coords_s.z) &&
					PAD::IS_CONTROL_RELEASED(2, 61)) VEHICLE::SET_VEHICLE_CHEAT_POWER_INCREASE(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0), 0.3f);
				if (VEH_ENG_POW_VALUES[engPowMultIndex] < 0 && VEHICLE::IS_THIS_MODEL_A_CAR(ENTITY::GET_ENTITY_MODEL(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0))) && !INTERIOR::IS_COLLISION_MARKED_OUTSIDE(my_coords_s.x, my_coords_s.y, my_coords_s.z) &&
					PAD::IS_CONTROL_RELEASED(2, 61)) VEHICLE::SET_VEHICLE_CHEAT_POWER_INCREASE(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0), 1.0f);
				no_grip_snowing_e = true;
			}
			// Slippery When Wet
			if (VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripRainingCustomIndex] > 0) {
				if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 1420204096 || MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3061285535 || MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 1840358669 ||
					MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3373937154) { // GET_NEXT_WEATHER_TYPE_HASH_NAME
					...
					if (slippery_r < slip_index && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_slip_r.x, coords_slip_r.y, coords_slip_r.z)) VEHICLE::SET_VEHICLE_REDUCE_GRIP(bus_veh[i], true);
					...
					if (VEH_ENG_POW_VALUES[engPowMultIndex] < 0 && VEHICLE::IS_THIS_MODEL_A_CAR(ENTITY::GET_ENTITY_MODEL(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0))) && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(my_coords_sl.x, my_coords_sl.y, my_coords_sl.z) &&
						PAD::IS_CONTROL_RELEASED(2, 61)) VEHICLE::SET_VEHICLE_CHEAT_POWER_INCREASE(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0), 0.2f);
					if (VEH_ENG_POW_VALUES[engPowMultIndex] < 0 && VEHICLE::IS_THIS_MODEL_A_CAR(ENTITY::GET_ENTITY_MODEL(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0))) && !INTERIOR::IS_COLLISION_MARKED_OUTSIDE(my_coords_sl.x, my_coords_sl.y, my_coords_sl.z) &&
						PAD::IS_CONTROL_RELEASED(2, 61)) VEHICLE::SET_VEHICLE_CHEAT_POWER_INCREASE(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0), 1.0f);
			...
			if ((VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripSnowingCustomIndex] > 0 && !featureSnow.enabled && no_grip_snowing_e == true) || (VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripSnowingCustomIndex] == 0 && featureSnow.enabled && no_grip_snowing_e == true) ||
				(VEH_TURN_SIGNALS_ACCELERATION_VALUES[RadarReducedGripRainingCustomIndex] > 0 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() != 1420204096 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() != 3061285535 &&
				MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() != 1840358669 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() != 3373937154 && no_grip_when_wet_e == true) || 
				...
				VEHICLE::SET_VEHICLE_CHEAT_POWER_INCREASE(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0), 1.0f);
			...
			if (featureHeadlightsBlackout && featureBlackout.enabled && VEHICLE::GET_IS_VEHICLE_ENGINE_RUNNING(bus_veh[i])) {
				...
				if (VEHICLE::IS_TOGGLE_MOD_ON(bus_veh[i], 22)) {
					r = 31;	g = 44; b = 54;
					if (getGameVersion() > 45) {
						int curColour = VEHICLE::GET_VEHICLE_XENON_LIGHT_COLOR_INDEX(bus_veh[i]);
				...
				//ENTITY::SET_ENTITY_INVINCIBLE(vehpolicelights, true, FALSE);
			...
			if (NPC_RAGDOLL_VALUES[NPCVehicleDamageOnCollIndex] > 0 && PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), 0)) {
				...
				if (bus_veh[i] != PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), false) && VEHICLE::GET_PED_IN_VEHICLE_SEAT(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), false), -1, FALSE) == PLAYER::PLAYER_PED_ID() &&
					...
					if (NPC_RAGDOLL_VALUES[NPCVehicleDamageOnCollIndex] == 2) {
						if (!FIRE::IS_ENTITY_ON_FIRE(VEHICLE::GET_PED_IN_VEHICLE_SEAT(bus_veh[i], -1, FALSE))) {
							FIRE::START_ENTITY_FIRE(VEHICLE::GET_PED_IN_VEHICLE_SEAT(bus_veh[i], -1, FALSE));
	...
	if (NPC_RAGDOLL_VALUES[NoPedsGravityIndex] > 0 || featureAcidWater || featureAcidRain || PLAYER_HEALTH_VALUES[PedsHealthIndex] > 0 ||
		...
		for (int i = 0; i < found_ped; i++) {
			...
			if (PLAYER_HEALTH_VALUES[PedsHealthIndex] > 0) { 
				if (ENTITY::GET_ENTITY_HEALTH(bus_ped[i]) == ENTITY::GET_ENTITY_MAX_HEALTH(bus_ped[i])) {
					if (bus_ped[i] != PLAYER::PLAYER_PED_ID() && !PED::IS_PED_GROUP_MEMBER(bus_ped[i], myENTGroup)) {
						...
						ENTITY::SET_ENTITY_HEALTH(bus_ped[i], PLAYER_HEALTH_VALUES[PedsHealthIndex], 0, 0);
			...
			if (featureNPCShowHealth && ENTITY::DOES_ENTITY_EXIST(bus_ped[i]) && !ENTITY::IS_ENTITY_DEAD(bus_ped[i], FALSE) && ENTITY::GET_ENTITY_HEALTH(bus_ped[i]) != ENTITY::GET_ENTITY_MAX_HEALTH(bus_ped[i]) &&
				...
				HUD::ADD_TEXT_COMPONENT_SUBSTRING_KEYBOARD_DISPLAY((char *)curr_h_t.c_str());
			...
			if (NPC_RAGDOLL_VALUES[NoPedsGravityIndex] > 0 && bus_ped[i] != PLAYER::PLAYER_PED_ID() && !PED::IS_PED_IN_ANY_VEHICLE(bus_ped[i], false)) {
				...
				if (NPC_RAGDOLL_VALUES[NoPedsGravityIndex] == 2) {
					...
					if (!ENTITY::IS_ENTITY_PLAYING_ANIM(bus_ped[i], "dead@fall", "dead_fall_down", 3) && !PED::IS_PED_RAGDOLL(bus_ped[i]) && PED::GET_PED_TYPE(bus_ped[i]) != 6 && !ENTITY::HAS_ENTITY_COLLIDED_WITH_ANYTHING(bus_ped[i])) {
						TASK::TASK_PLAY_ANIM(bus_ped[i], "dead@fall", "dead_fall_down", 8.0, 0.0, -1, 9, 0, 0, 0, 0);
			...
			if (featureAcidWater && (ENTITY::IS_ENTITY_IN_WATER(bus_ped[i]) || PED::IS_PED_SWIMMING_UNDER_WATER(bus_ped[i]))) {
				...
				if (PED::GET_PED_ARMOUR(bus_ped[i]) > 0) {
					if (!AUDIO::IS_AMBIENT_SPEECH_PLAYING(bus_ped[i])) AUDIO::PLAY_PED_AMBIENT_SPEECH_NATIVE(bus_ped[i], "BLOCKED_GENERIC", "SPEECH_PARAMS_FORCE_SHOUTED", 0);
				...
				if (ENTITY::GET_ENTITY_HEALTH(bus_ped[i]) > 0 && PED::GET_PED_ARMOUR(bus_ped[i]) < 1) {
					...
					if (bus_ped[i] != PLAYER::PLAYER_PED_ID() && acid_counter > 4) { 
						ENTITY::SET_ENTITY_HEALTH(bus_ped[i], ENTITY::GET_ENTITY_HEALTH(bus_ped[i]) - 1, 0, 0);
						acid_counter = 0;
					}
			...
			if (featureAcidRain) {
				...
				int temp1 = SHAPETEST::START_EXPENSIVE_SYNCHRONOUS_SHAPE_TEST_LOS_PROBE(coords_ped.x, coords_ped.y, coords_ped.z, coords_ped.x, coords_ped.y, coords_ped.z + 2000, -1, bus_ped[i], 1);
				int result = SHAPETEST::GET_SHAPE_TEST_RESULT(temp1, &hit, &endCoords, &surfaceNormal, &entityHit);
				if (MISC::GET_RAIN_LEVEL() > 0 && INTERIOR::IS_COLLISION_MARKED_OUTSIDE(coords_ped.x, coords_ped.y, coords_ped.z) && hit == 0 && (!PED::IS_PED_IN_ANY_VEHICLE(bus_ped[i], 0) || (PED::IS_PED_IN_ANY_VEHICLE(bus_ped[i], 0) &&
					(VEHICLE::GET_CONVERTIBLE_ROOF_STATE(veh_currveh) == 2 || !VEHICLE::DOES_VEHICLE_HAVE_ROOF(veh_currveh) || PED::IS_PED_ON_ANY_BIKE(bus_ped[i]) || VEHICLE::IS_THIS_MODEL_A_QUADBIKE(currVehModel) ||
						VEHICLE::IS_THIS_MODEL_A_BICYCLE(currVehModel) || currVehModel == MISC::GET_HASH_KEY("BODHI2") || currVehModel == MISC::GET_HASH_KEY("AIRTUG") || currVehModel == MISC::GET_HASH_KEY("CADDY3") ||
						currVehModel == MISC::GET_HASH_KEY("MOWER") || currVehModel == MISC::GET_HASH_KEY("TRACTOR") || currVehModel == MISC::GET_HASH_KEY("THRUSTER") || currVehModel == MISC::GET_HASH_KEY("DUSTER") ||
						currVehModel == MISC::GET_HASH_KEY("TORO") || currVehModel == MISC::GET_HASH_KEY("DINGHY2") || currVehModel == MISC::GET_HASH_KEY("DINGHY3") || currVehModel == MISC::GET_HASH_KEY("DINGHY") ||
						currVehModel == MISC::GET_HASH_KEY("SPEEDER") || currVehModel == MISC::GET_HASH_KEY("JETMAX") || currVehModel == MISC::GET_HASH_KEY("SQUALO") || currVehModel == MISC::GET_HASH_KEY("SUNTRAP") ||
						currVehModel == MISC::GET_HASH_KEY("SEASHARK") || currVehModel == MISC::GET_HASH_KEY("SEASHARK2"))))) {
					if (bus_ped[i] != PLAYER::PLAYER_PED_ID()) acid_counter = acid_counter + 1; 
					if (bus_ped[i] == PLAYER::PLAYER_PED_ID()) { 
						s_tick_secs_passed = clock() / CLOCKS_PER_SEC;
						if (((clock() / (CLOCKS_PER_SEC / 1000)) - s_tick_secs_curr) != 0) {
			...
			if (NPC_RAGDOLL_VALUES[CopBlipPermIndex] > 0 && (PED::GET_PED_TYPE(bus_ped[i]) == 6 || PED::GET_PED_TYPE(bus_ped[i]) == 27)) { // ENTITY::DOES_ENTITY_EXIST(bus_ped[i])
				if (!COP_VECTOR.empty()) {
					...
					for (int j = 0; j < COP_VECTOR.size(); j++) {
						...
						if (!ENTITY::DOES_ENTITY_EXIST(COP_VECTOR[j]) || PED::IS_PED_DEAD_OR_DYING(COP_VECTOR[j], true)) { // && HUD::DOES_BLIP_EXIST(BLIPTABLE_COP[i])
							HUD::REMOVE_BLIP(&BLIPTABLE_COP[j]);
					...
					if (exists_already == false && !PED::IS_PED_DEAD_OR_DYING(bus_ped[i], true)) {
						blip_cops = HUD::ADD_BLIP_FOR_ENTITY(bus_ped[i]);
						HUD::SET_BLIP_SPRITE(blip_cops, 41); // 42
						HUD::SET_BLIP_SCALE(blip_cops, 0.5);
						if (NPC_RAGDOLL_VALUES[CopBlipPermIndex] == 1) HUD::SET_BLIP_AS_SHORT_RANGE(blip_cops, true);
	...
	if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] > 0) {
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 1 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_BasejumpHeli");
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_BasejumpPack");
			blipIterator = BlipSpriteBaseJump;
			for (Blip i = HUD::GET_FIRST_BLIP_INFO_ID(blipIterator); HUD::DOES_BLIP_EXIST(i) != 0; i = HUD::GET_NEXT_BLIP_INFO_ID(blipIterator)) HUD::REMOVE_BLIP(&i);
		}
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 2 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_OffroadRacing");
			blipIterator = BlipSpriteOffRoadRaceFinish;
			for (Blip i = HUD::GET_FIRST_BLIP_INFO_ID(blipIterator); HUD::DOES_BLIP_EXIST(i) != 0; i = HUD::GET_NEXT_BLIP_INFO_ID(blipIterator)) HUD::REMOVE_BLIP(&i);
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_Racing");
			blipIterator = BlipSpriteRace;
			for (Blip i = HUD::GET_FIRST_BLIP_INFO_ID(blipIterator); HUD::DOES_BLIP_EXIST(i) != 0; i = HUD::GET_NEXT_BLIP_INFO_ID(blipIterator)) HUD::REMOVE_BLIP(&i);
			blipIterator = BlipSpriteRaceSea;
			for (Blip i = HUD::GET_FIRST_BLIP_INFO_ID(blipIterator); HUD::DOES_BLIP_EXIST(i) != 0; i = HUD::GET_NEXT_BLIP_INFO_ID(blipIterator)) HUD::REMOVE_BLIP(&i);
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_stunts");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 3 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_darts");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 4 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_golf");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 5 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_Hunting_Ambient");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 6 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_pilotschool");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 7 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_range");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 8 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_tennis");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 9 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_Triathlon");
		...
		if (WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 10 || WORLD_FREEROAM_ACTIVITIES_VALUES[featureFreeroamActivitiesIndex] == 11) {
			MISC::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("launcher_Yoga");
	...
	if (featureNoGameHintCameraLocking && CAMERA::IS_GAMEPLAY_HINT_ACTIVE() && MISC::GET_MISSION_FLAG() == 0) CAMERA::STOP_GAMEPLAY_HINT(true);

	// Wind Strength
	if (windstrength_toggle == false) {
		MISC::SET_WIND(WORLD_WIND_STRENGTH_VALUES[WindStrengthIndex]);
		if (WORLD_WIND_STRENGTH_VALUES[WindStrengthIndex] != 0) MISC::SET_WIND_DIRECTION(ENTITY::GET_ENTITY_HEADING(PLAYER::PLAYER_PED_ID()));
		if (WORLD_WIND_STRENGTH_VALUES[WindStrengthIndex] == 0) MISC::SET_WIND(0.0); // MISC::SET_WIND_SPEED(0.0);
	...
	// No Waypoint
	if (featureNoWaypoint) HUD::CLEAR_GPS_PLAYER_WAYPOINT();

	// No Traffic
	if (featureWorldNoTraffic.updated)
	{
		VEHICLE::SET_DISTANT_CARS_ENABLED(!featureWorldNoTraffic.enabled);
		GRAPHICS::DISABLE_VEHICLE_DISTANTLIGHTS(featureWorldNoTraffic.enabled);

		if (featureWorldNoTraffic.enabled)
		{
			Vector3 v3 = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 1);
			MISC::CLEAR_AREA_OF_VEHICLES(v3.x, v3.y, v3.z, 1000.0, 0, 0, 0, 0, 0, FALSE, 0);
	...
	if (featureNoPlanesHelis) {
		...
		TASK::SET_SCENARIO_GROUP_ENABLED("ng_planes", 0);
		TASK::SET_SCENARIO_TYPE_ENABLED("WORLD_VEHICLE_MILITARY_PLANES_SMALL", 0);
		TASK::SET_SCENARIO_TYPE_ENABLED("WORLD_VEHICLE_MILITARY_PLANES_BIG", 0);
		...
		for (char* cur_aircraft : aircrafts) {
			VEHICLE::SET_VEHICLE_MODEL_IS_SUPPRESSED(MISC::GET_HASH_KEY(cur_aircraft), 1);
		}
	}
	...
	if (featureNoAnimals) {
		...
		TASK::SET_SCENARIO_GROUP_ENABLED("ng_birds", 0);
		...
		for (char* cur_ainimal : animals) {
			PED::SET_PED_MODEL_IS_SUPPRESSED(MISC::GET_HASH_KEY(cur_ainimal), 1);
		}
		no_animals = true;
	}
	...
	if (featureWeatherFreeze.enabled && featureWeatherFreeze.updated == false) {
		...
		if (!lastWeather.empty()) {
			if (MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3061285535) MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
			if (WORLD_LIGHTNING_INTENSITY_VALUES[featureLightIntensityIndex] > -2 && MISC::GET_PREV_WEATHER_TYPE_HASH_NAME() == 3373937154) MISC::LOAD_CLOUD_HAT("Stormy 01", 0.3);
			MISC::SET_WEATHER_TYPE_NOW_PERSIST((char*)lastWeather.c_str());
		}
	...
	if (featureWeatherFreeze.enabled && (DLC::GET_IS_LOADING_SCREEN_ACTIVE() || (time_since_d > -1 && time_since_d < 2000))) featureWeatherFreeze.updated = false;
	...
	// Freeze Clouds
	if (featureCloudsFreeze && !lastClouds.empty()) MISC::LOAD_CLOUD_HAT((char *)lastClouds.c_str(), 0.3);
	if (featureCloudsFreeze && lastClouds.empty()) MISC::UNLOAD_ALL_CLOUD_HATS();
	...
	if (featureSnow.updated)
	{
		if (featureSnow.enabled)
		{
			...
			EnableTracks(true, true, true);
			// THANKS TO ALTSIERRA117 FOR THE ORIGINAL CODE
			STREAMING::REQUEST_NAMED_PTFX_ASSET("core_snow");
			GRAPHICS::USE_PARTICLE_FX_ASSET("core_snow");
			AUDIO::REQUEST_SCRIPT_AUDIO_BANK("ICE_FOOTSTEPS", true, 0);
			AUDIO::REQUEST_SCRIPT_AUDIO_BANK("SNOW_FOOTSTEPS", true, 0);
		}
		else
		{
			EnableSnow(featureSnow.enabled, firstWorldFeaturesUpdatePass);
			EnableTracks(false, false, false);
	...
	// GRAPHICS::USE_SNOW_FOOT_VFX_WHEN_UNSHELTERED/USE_SNOW_WHEEL_VFX_WHEN_UNSHELTERED are
	// per-frame "force" flags (their native DB aliases are _SET_FORCE_PED_FOOTSTEPS_TRACKS/
	// _SET_FORCE_VEHICLE_TRAILS) - the game resets them every tick, so calling them only once
	// inside the featureSnow.updated block above only affects whatever's nearby in that exact
	// frame. Re-assert every tick like SET_GRAVITY_LEVEL above so tracks keep appearing for
	// peds/vehicles that pass through later, not just at the moment Heavy Snow was toggled.
	GRAPHICS::USE_SNOW_FOOT_VFX_WHEN_UNSHELTERED(featureSnow.enabled);
	GRAPHICS::USE_SNOW_WHEEL_VFX_WHEN_UNSHELTERED(featureSnow.enabled);

	if (featureMPMap.enabled) {
		if (featureMPMap.updated == true && MISC::GET_MISSION_FLAG() == 0) {
...
void EnableTracks(bool tracksVehicle = false, bool tracksPeds = false, bool deepTracksVehicle = false)
{
	// Was two raw byte patches (VAR_FeetSnowTracks/VAR_VehicleSnowTracks, found via
	// ScanPattern) - GRAPHICS::USE_SNOW_FOOT_VFX_WHEN_UNSHELTERED/USE_SNOW_WHEEL_VFX_WHEN_UNSHELTERED
	// are natives that do the same thing (per the native DB, foot VFX is documented as
	// "Forces footstep tracks on all surfaces"), so this now works on both games instead
	// of needing its own Enhanced pattern. This also fixes a latent bug: the old code
	// wrote tracksVehicle into VAR_FeetSnowTracks and tracksPeds into
	// VAR_VehicleSnowTracks - backwards versus both variable and parameter names -
	// invisible until now only because every call site passed the same value for both.
	GRAPHICS::USE_SNOW_FOOT_VFX_WHEN_UNSHELTERED(tracksPeds);
	GRAPHICS::USE_SNOW_WHEEL_VFX_WHEN_UNSHELTERED(tracksVehicle);

	// Switch for big/small tracks
	static auto vehicleTrackTypes = ScanPattern("B9 ? ? ? ? 84 C0 44 0F 44 F1").As<uintptr_t>();
...
//
// Used to also support pre-3095.0 Legacy builds via a byte-patch fallback (NOPing/jmp-patching
// the code that suppresses ground snow, restored on disable) for versions where
// _FORCE_GROUND_SNOW_PASS didn't exist yet. Removed: every version in eGameVersion newer than
// that - i.e. every Legacy build anyone still runs - takes the native path, so the fallback was
// dead code kept alive only by the version branch.
